...
def normalize_financial_data(df):
    ...
    df["norm_price"] = (df["close"] - df["close"].rolling(20).mean()) / df[
        "close"
    ].rolling(20).std()
    return df
...
def create_financial_cnn(input_shape, num_classes):
    ...
    model = keras.Sequential(
        [
            ...
            keras.layers.Conv1D(
                64, kernel_size=3, activation="relu", input_shape=input_shape
            ),
...
def generate_financial_data(n_days=500, seed=42):
    ...
    df = pd.DataFrame(
        {"timestamp": dates, "close": prices, "volume": volume, "returns": returns}
    )
...
def main(plot: bool = False):
    ...
    logger.info(
        classification_report(y[test_idx], y_pred, target_names=["Bearish", "Bullish"])
    )
