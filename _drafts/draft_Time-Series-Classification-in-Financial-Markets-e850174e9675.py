...
class RiskManager:
    ...
    def calculate_position_size(self, prediction, current_equity):
        ...
        confidence = abs(prediction[0][1] - 0.5) * 2
...
def evaluate_trading_performance(returns, positions):
    ...
    metrics = {
        ...
        "profit_factor": abs(sum(returns[returns > 0]) / sum(returns[returns < 0]))
        if sum(returns < 0) != 0
        else np.inf,
...
def financial_dtw_distance(s1, s2, max_warping=0.1):
    ...
    """
    n, m = (len(s1), len(s2))
    max_warp = int(max_warping * min(n, m))
...
def generate_financial_data(n_days=500, seed=42):
    ...
    df = pd.DataFrame({"timestamp": dates, "close": prices, "volume": volume, "returns": returns})
    return df
...
def normalize_financial_data(df):
    ...
    df["norm_price"] = (df["close"] - df["close"].rolling(20).mean()) / df["close"].rolling(
        20
    ).std()
    return df
...
def generate_data() -> None:
    ...
    if plot:
        ...
        axes[0, 0].scatter(
            bullish_periods["timestamp"],
            bullish_periods["close"],
            c="green",
            alpha=0.3,
            s=10,
            label="Bullish",
        ...
        axes[0, 0].scatter(
            bearish_periods["timestamp"],
            bearish_periods["close"],
            c="red",
            alpha=0.3,
            s=10,
            label="Bearish",
        ...
        axes[1, 1].axhline(
            y=np.mean(scores), color="r", linestyle="--", label=f"Mean: {np.mean(scores):.4f}"
        )
